...
response = requests.post(f"http://localhost:8080/api/v1/embed", json=embed_data)
...
if response.status_code == 200:
    ...
    print("Doesn't Expire" if response.json()['expires'] is False else f"Does Expire: {response.json()['expires']}")
